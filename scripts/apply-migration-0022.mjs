...
const sql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/0022_status_public_rls.sql"),
  "utf8",
);
...
async function main() {
  ...
  for (const { label, ...config } of configs) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Applied migration 0022_status_public_rls.sql via ${label}`);
        return;
      } finally {
        await client.end();
      }
  ...
  console.error("Failed to apply migration 0022 with all connection candidates.");
  console.error(
