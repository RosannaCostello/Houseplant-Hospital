...
const sql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/0029_plant_internal_notes.sql"),
  "utf8",
);
...
async function main() {
  ...
  for (const { label, ...config } of configs) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Applied migration 0029_plant_internal_notes.sql via ${label}`);
        return;
      } finally {
        await client.end();
      }
  ...
  console.error("Failed to apply migration 0029 with all connection candidates.");
  console.error(
