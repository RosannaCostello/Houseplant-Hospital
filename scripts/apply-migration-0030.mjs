...
const sql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/0030_stacking_cards_setting.sql"),
  "utf8",
);
...
async function main() {
  ...
  for (const { label, ...config } of configs) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Applied migration 0030_stacking_cards_setting.sql via ${label}`);
        return;
      } finally {
        await client.end();
      }
  ...
  console.error("Failed to apply migration 0030 with all connection candidates.");
  console.error(
