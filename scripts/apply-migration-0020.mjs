...
const sql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/0020_pest_treatments.sql"),
  "utf8",
);
...
async function main() {
  ...
  for (const { label, ...config } of configs) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Applied migration 0020_pest_treatments.sql via ${label}`);
        return;
      } finally {
        await client.end();
      }
  ...
  console.error("Failed to apply migration 0020 with all connection candidates.");
  console.error(
