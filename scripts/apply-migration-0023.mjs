...
const sql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/0023_pest_treatment_options.sql"),
  "utf8",
);
...
async function main() {
  ...
  for (const { label, ...config } of configs) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Applied migration 0023_pest_treatment_options.sql via ${label}`);
        return;
      } finally {
        await client.end();
      }
  ...
  console.error("Failed to apply migration 0023 with all connection candidates.");
  console.error(
