...
const sql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/0035_propagate_allow_pests.sql"),
  "utf8",
);
...
async function main() {
  ...
  for (const { label, ...config } of configs) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Applied migration 0035_propagate_allow_pests.sql via ${label}`);
        return;
      } finally {
        await client.end();
      }
  ...
  console.error("Failed to apply migration 0035 with all connection candidates.");
  console.error(
