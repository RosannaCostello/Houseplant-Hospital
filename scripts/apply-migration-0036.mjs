...
const sql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/0036_drop_propagation_no_pests_check.sql"),
  "utf8",
);
...
async function main() {
  ...
  for (const { label, ...config } of configs) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Applied migration 0036_drop_propagation_no_pests_check.sql via ${label}`);
        return;
      } finally {
        await client.end();
      }
  ...
  console.error("Failed to apply migration 0036 with all connection candidates.");
  console.error(
