...
const sql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/0028_rename_size_xs_to_mini.sql"),
  "utf8",
);
...
async function main() {
  ...
  for (const { label, ...config } of configs) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Applied migration 0028_print_jobs_delivery.sql via ${label}`);
        return;
      } finally {
        await client.end();
      }
  ...
  console.error("Failed to apply migration 0028 with all connection candidates.");
  console.error(
