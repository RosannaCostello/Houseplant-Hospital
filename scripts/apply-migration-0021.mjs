...
const sql = readFileSync(
  resolve(process.cwd(), "supabase/migrations/0021_preprod_hardening.sql"),
  "utf8",
);
...
async function main() {
  ...
  for (const { label, ...config } of configs) {
    ...
    try {
      ...
      try {
        ...
        console.log(`Applied migration 0021_preprod_hardening.sql via ${label}`);
        return;
      } finally {
        await client.end();
      }
  ...
  console.error("Failed to apply migration 0021 with all connection candidates.");
  console.error(
