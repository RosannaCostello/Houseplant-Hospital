    ...
    >
      <div className="relative h-20 w-24 shrink-0 overflow-hidden rounded-none border border-hilda-border/10 bg-hilda-bg">
        {plant.thumbnailUrl ? (
          ...
          <div className="flex h-full items-center justify-center text-[10px] font-medium uppercase tracking-wide text-hilda-text-muted">
            No photo
          </div>
        )}
        {plant.bugsFound ? (
          <BugsFoundBadge className="absolute right-1 top-1 bg-hilda-bugs" iconClassName="h-3 w-3" />
        ) : null}
      </div>
...
export function VisitDetailView({ visit }: VisitDetailViewProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-8">
      <div>
        <Link href="/app" className="text-sm font-medium text-hilda-text hover:text-hilda-heading">
          ← Back to dashboard
        </Link>
        <h1 className="mt-3 text-2xl font-semibold tracking-tight text-hilda-heading sm:text-3xl">
          {visit.customer.firstName} {visit.customer.lastName}
        </h1>
        <p className="mt-1 text-base text-hilda-text">
          Drop-off ·{" "}
          {new Date(visit.checkinDate).toLocaleString("en-GB", {
            dateStyle: "medium",
            timeStyle: "short",
          })}{" "}
          <span className="text-hilda-text-muted">({formatPlantAge(visit.checkinDate)})</span>
        </p>
      </div>
      ...
      <dl className="grid gap-4 rounded-none border border-hilda-border/15 bg-hilda-surface p-5 shadow-sm sm:grid-cols-2">
        ...
        <div>
          ...
          <dd className="mt-1 text-sm font-medium text-hilda-heading">
            {plantCount} {plantCount === 1 ? "plant" : "plants"}
          </dd>
        </div>
        <div className="sm:col-span-2">
          <dt className="text-xs font-medium uppercase tracking-wide text-hilda-text-muted">Contact</dt>
          <dd className="mt-1 text-sm text-hilda-heading">
